...
def extract_audio_in_video(input_video_path):
    ...
    return os.system(
        "ffmpeg -y -i "
        + input_video_path
        + " -vn -ar 44.1k -ac 1 -ab 256k extract_audio/all_extract_audio.wav"
    )
...
def extract_image_in_video(input_video_path):
    ...
    return os.system(
        ...
        + input_video_path
        + " -y -vframes 1 -an -s 1280x720 output/"
        + input_video_path[input_video_path.find("/") + 1 : input_video_path.find(".")]
        + ".jpg"
    )
...
def divide_audio(divide_time):
    ...
    for i in range(segment_count):
        audio_divided = entire_audio[
            i * segment_len * 1000 : i * segment_len * 1000 + segment_len * 1000
        ]
        path_and_startend_0 += [
            [
                "extract_audio/output0/output0_" + str(i) + ".wav",
                i * segment_len,
                i * segment_len + segment_len,
            ]
        ]
        audio_divided.export(
            "extract_audio/output0/output0_" + str(i) + ".wav", format="wav"
        )
...
def mask_video(start_and_end_time):
    """
    욕을 말한 시작점과 끝점을 입력받아 욕을 한 시점에 입 모양을 가립니다.
    :param start_and_end_time: 욕을 말한 시작점과 끝점입니다.
    """
    pass
...
def combine_audio_and_video(video_path):
    ...
    return os.system(
        "ffmpeg -y -i "
        + video_path
        + " -i detector_result/audio_result.wav -c:v copy -c:a aac -strict experimental -map 0:v:0 -map 1:a:0 output/"
        + video_path[video_path.find("/") + 1 : video_path.find(".")]
        + ".mp4"
    )
...
def get_all_data():
    ...
    list_result_detector = [
        name for name in list_result_detector if name.endswith("txt")
    ]
